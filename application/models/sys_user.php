...
class sys_user extends MY_Model {
	...
	public function _login($post_token, $password = null){
		$this->db->flush_cache();
		$this->db->select('sys_user.*, sys_group.name as group_name, t_ledging.name as ledging');
		$this->db->from($this->table);
		$this->db->join('sys_group', 'sys_user.group = sys_group.id');
		$this->db->join('t_ledging', 'sys_user.id_ledging = t_ledging.id', 'left');
		if($password == null){
			$this->db->like('token', addcslashes($post_token, '%_'), 'none');
		}else{
			$this->db->where("(`username` LIKE " . $this->db->escape($post_token) . " OR `email` LIKE " . $this->db->escape($post_token) . ") AND `password` LIKE " . addcslashes($this->db->escape($password), '%_'));
		}
		$q = $this->db->get();
		if($q->num_rows() > 0){
			$user = $q->row();
	...
	public function get_by_email($email){
		...
		$this->db->select('sys_user.*, sys_group.name as group_name, t_ledging.name as ledging');
		$this->db->join('sys_group', 'sys_user.group = sys_group.id');
		$this->db->join('t_ledging', 'sys_user.id_ledging = t_ledging.id');
	...
	public function select_operator_only($limit = 0, $offset = 0){
		$this->db->flush_cache();
		$this->db->select('sys_user.id, sys_user.name, sys_user.username, sys_user.email, sys_user.password, (select name from sys_group where sys_group.id = sys_user.`group`) `group`, (select name from t_ledging where t_ledging.id = sys_user.id_ledging) ledging, (select name from t_cabor where t_cabor.id = sys_user.id_cabor) cabor');
		$this->db->where('sys_user.group = 3');
		if($limit > 0){
			if($offset > 0)
				$this->db->limit($limit, $offset);
			else
